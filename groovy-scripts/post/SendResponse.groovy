...
public class SendResponse extends TitanFilter {
	private static final DynamicIntProperty BUFFER_SIZE = DynamicPropertyFactory.getInstance().getIntProperty(Constants.TITAN_INITIAL_STREAM_BUFFER_SIZE, 1024);

    private static final DynamicBooleanProperty CONTENT_LENGTH = DynamicPropertyFactory.getInstance().getBooleanProperty(Constants.TITAN_SET_CONTENT_LENGTH, false);
	...
	public Object run() {
		...
		try{
			...
			final String requestEncoding = context.getRequest().getHeader(TitanHeaders.ACCEPT_ENCODING)
			if (requestEncoding != null && requestEncoding.contains("gzip")) {
				isGZipRequested = true;
			}
			if (isGZipRequested) {
				servletResponse.setHeader(TitanHeaders.CONTENT_ENCODING, "gzip")
			} else {
				try {
	...
	private void addResponseHeaders() {
		RequestContext context = RequestContext.getCurrentContext();
		HttpServletResponse servletResponse = context.getResponse();
		List<Pair<String, String>> titanResponseHeaders = context.getTitanResponseHeaders();

		titanResponseHeaders?.each { Pair<String, String> it ->
